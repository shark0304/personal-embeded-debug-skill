...
def test_create_failure_notebook(tmp_path: Path) -> None:
    ...
    assert (case_dir / "case_status.json").is_file()
    assert (case_dir / "lifecycle.md").is_file()
    assert (case_dir / "evidence.md").is_file()
    assert (case_dir / "fix_verification.md").is_file()
